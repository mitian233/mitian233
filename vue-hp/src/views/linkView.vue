...
<div class="flex flex-row justify-center">
  <div class="flex flex-col justify-center">
    <div class="card bg-base-100 shadow-2xl mx-10 max-w-4xl">
      <div class="card-body">
        ...
        <div class="flex flex-wrap justify-center">
          ...
          <div v-else v-for="(card, index) in links">
            <a :href="card.siteURL">
              <div class="relative rounded-xl overflow-auto p-3 md:w-96 w-64">
                <div class="overflow-hidden relative max-w-md mx-auto bg-white shadow-lg ring-1 ring-black/5 rounded-xl flex items-center gap-6 dark:bg-slate-800 dark:highlight-white/5">
                  <img v-if="card.icon === ''" class="absolute -left-6 w-28 h-28 rounded-full shadow-lg" src="@/assets/images/default.jpg" :id="'avatar'+index"/>
                  <img v-else class="absolute -left-6 w-28 h-28 rounded-full shadow-lg" :src="card.icon" :id="'avatar'+index" :key="index"/>
        ...
        <p class="text-xs text-right">📧: <a class="link" href="mailto:kasumi@bangdream.moe">kasumi@bangdream.moe</a></p>
      </div>
    </div>
  </div>
</div>
